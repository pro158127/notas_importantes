aprender a usar cada pin y su uso especifico 

1)Lo primero que vamos a hacer es hacer un  ejerccios de encender un led .

materiales:
1 LED

1 Resistencia 


Arduino Uno


conceptos a aprender primero  en codificacion:
estrucutura:

##setup()
pinMode()


## Loop()

digitalWrite()

¿Que es un pin digital y que pines son digitales ?
un pin digital es un puertoel cualfunciona con dos estdos encendido(High ) ,Bajo(Low)

## Para que sirven?
sirven para conectar y controlar ---> LED, BOTONES ,REALE Y MOTORES


